...
export function ItemSections({ listId, empty }: { listId: string; empty: ComponentChildren }) {
  ...
  return (
    <>
      {open.value.length > 0 && <Card items={open.value} />}
      {done.value.length > 0 && (
        <section class="mt-8">
          <SectionTitle
            action={
...
export function Card({ items, faded = false }: { items: Item[]; faded?: boolean }) {
  return (
    <ul
  ...
  return (
    <li class="flex items-center">
      <label class="flex min-w-0 flex-1 cursor-pointer items-center gap-3 py-3.5 pl-4 select-none">
        <input type="checkbox" checked={item.checked} onChange={() => setChecked(item, !item.checked)} class="peer sr-only" />
